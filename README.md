...
Метод ***void ArrayPrint(int[] arr)*** получает в качестве входного аргумента целочисленный массив *arr* и выводит последовательно, в цикле, значения элементов через пробел в консоль. Метод можно было реализовать в виде функции типа *string* - перевести элементы в строку, которую затем выдать как результат работы метода. Но при большой длине массива такой вариант не очень хорош, т.к. тип *string* имеет ограничения по длине строки.  